...
def batch_inference(model, path, cfg, test=False, write=False, fail_cases=False):
    ...
    stats.to_csv(osp.join(write_dir, 'run2_test_results.csv'), index=False)

    with open(osp.join(write_dir, 'run2_test_results.csv'), 'a') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pt', '--bestpt', default='run2/train/weights/best.pt')
